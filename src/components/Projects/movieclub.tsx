...
import { movieclubData, movieclubProjectDetails } from "../../Utils/movieclubData";
import { useTitle } from "../../Hooks/useTitle";

export default function MovieClub(): JSX.Element {
  useTitle("MovieClub");
  ...
  return (
    <div className="min-h-screen mt-16 bg-gray-50 dark:bg-dark-second px-4 lg:px-8">
     ...
     Full-Stack Development Project - MovieClub
      </h2>
      <ImageCarousel data={movieclubData} />

      <div className="flex items-center justify-center mt-4">
        <ul className="list-none space-y-8 text-base sm:text-lg text-gray-700 dark:text-gray-300 w-full max-w-6xl">
          {movieclubProjectDetails.map(({ title, images }, index) => (
            <div
              key={index}
                      ...
                      images.map((image, imgIndex) => (
                        <Zoom key={imgIndex}>
                          ...
                          >
                            <img
                              src={image}
                              alt={`MovieClub ${title} ${imgIndex + 1}`}
